...
bool read_screen_state(CPU &cpu, FrameBuffer &frame_buffer) {
  ...
  for (int i = 0x0200; i < 0x600; i++) {
    ...
    if (frame_buffer[frame_idx]     != color.r ||
...
void update_key_state() {
  if      (IsKeyDown(KEY_W)) last_key = 0x77;
  else if (IsKeyDown(KEY_S)) last_key = 0x73;
  else if (IsKeyDown(KEY_A)) last_key = 0x61;
  else if (IsKeyDown(KEY_D)) last_key = 0x64;
}
...
int main() {
  ...
  Texture2D texture = LoadTextureFromImage(
      GenImageColor(SCREEN_WIDTH, SCREEN_HEIGHT, BLACK));
