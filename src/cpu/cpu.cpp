...
CPU::CPU() : pc(0), sp(STACK_RESET), reg_a(0), reg_x(0), reg_y(0), status(flags::UNUSED) {}

void CPU::load_program(const std::vector<uint8_t> &program, uint16_t start_addr) {
...
// flag utils
void CPU::set_flag(uint8_t mask, bool condition) { condition ? status |= mask : status &= ~mask; }
...
void CPU::write_to_reg_a_or_mem(AddressingMode mode, std::optional<uint16_t> addr, uint8_t value) {
    if (mode == AddressingMode::Accumulator) {
        set_register_a(value);
    } else {
        if (!addr.has_value()) {
